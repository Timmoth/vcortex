...
public class RMSpropOptimizer : IOptimizer
{
        ...
        ArrayView<float> movingAverageOfSquares)
    {
        ...
        parameters[index] -= input.LearningRate * gradientSum / (MathF.Sqrt(movingAverageOfSquares[index]) + input.Epsilon);
    }
    ...
    public void Compile(NetworkTrainer trainer)
    {
        ...
        _movingAverageOfSquares = trainer.Accelerator.Allocate1D<float>(trainer.Network.ParameterCount);
        _optimizerKernel =
            trainer.Accelerator
                .LoadAutoGroupedStreamKernel<Index1D, OptimizerKernelInput, ArrayView<float>, ArrayView<float>, ArrayView<float>>(RMSpropOptimizerKernelImpl);
    }
    ...
    public void Optimize(NetworkData networkData, NetworkAcceleratorBuffers buffers, float learningRate)
    {
        ...
        _optimizerKernel(networkData.ParameterCount, _optimizerKernelInput, buffers.Parameters.View, buffers.Gradients.View, _movingAverageOfSquares.View);
    }

    public void Dispose() { _movingAverageOfSquares.Dispose(); }
}